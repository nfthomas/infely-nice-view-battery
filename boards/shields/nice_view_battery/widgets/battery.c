...
void draw_battery_status(lv_obj_t *canvas, const struct status_state *state) {
    ...
    lv_draw_label_dsc_t label_background_dsc;
    init_label_dsc(&label_background_dsc, LVGL_BACKGROUND, &font, LV_TEXT_ALIGN_CENTER);
    ...
    lv_canvas_draw_text(canvas, 0 - 1, y - 1, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0, y - 1, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0 + 1, y - 1, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0 - 1, y, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0 + 1, y, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0 - 1, y + 1, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0, y + 1, w, &label_background_dsc, text);
    lv_canvas_draw_text(canvas, 0 + 1, y + 1, w, &label_background_dsc, text);
